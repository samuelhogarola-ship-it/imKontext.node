...
/* ── STATE ───────────────────────────────────────────────────── */
let allTexts       = [];   // [{id, title, slug, levels:[]}]
let selectedText   = null; // {id, title, slug}
...
async function goToTextos() {
  ...
  try {
    // 1. Get all texts
    const texts = await sbFetch('texts?select=id,title,slug&order=id.asc');
...
function selectText(text) {
  ...
  // Update header
  $('content-title').textContent = text.title;
  $('content-description').textContent =
    `Lee el artículo "${text.title}" y vuelve cuando estés listo para practicar el vocabulario.`;

  // Build text URL (assumes texts live at imkontext.vokabellab.com/{slug})
  const textUrl = `https://imkontext.vokabellab.com/${text.slug}`;
  $('btn-open-text').href = textUrl;

  // Also update activity title for step 3
  $('act-text-title').textContent = text.title;
