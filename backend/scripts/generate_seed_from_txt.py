...
def build_sql(themes: list[dict], input_path: Path) -> str:
    ...
    lines.extend(
        [
            ...
            "--         vocabulary_lemmas, vocabulary_meanings,",
            "--         text_version_meanings, app_vocabulary_meanings",
            "-- ============================================================",
            "",
        ]
    )
    ...
    # --- Core sync block --------------------------------------------------
    # Keeps vocabulary_lemmas, vocabulary_meanings, text_version_meanings
    # and app_vocabulary_meanings in sync with every new seed so imKontext
    # continues to serve vocabulary from the unified core after ingestion.
    lines.extend([
        "-- ============================================================",
        "-- Core sync: vocabulary_lemmas / vocabulary_meanings /",
        "--            text_version_meanings / app_vocabulary_meanings",
        "-- Idempotent — safe to re-run on existing data.",
        "-- ============================================================",
        "",
        "-- 1. New lemmas",
        "insert into vocabulary_lemmas (german, normalized_german, article, word_type, conjugation, is_active)",
        "select distinct on (lower(v.german))",
        "  v.german,",
        "  lower(trim(regexp_replace(v.german, '^(der|die|das)\\s+', '', 'i'))),",
        "  v.article,",
        "  v.word_type,",
        "  case",
        "    when v.infinitive is not null then",
        "      jsonb_build_object(",
        ...
        "  true",
        "from vocabulario v",
        "where v.lemma_id is null",
        "  and not exists (",
        "    select 1 from vocabulary_lemmas vl",
        "    where lower(vl.german) = lower(v.german)",
        "  );",
        "",
        "-- 2. Back-fill vocabulario.lemma_id",
        "update vocabulario v",
        "set lemma_id = vl.id",
        "from vocabulary_lemmas vl",
        "where lower(vl.german) = lower(v.german)",
        "  and v.lemma_id is null;",
        "",
        "-- 3. New meanings",
        "insert into vocabulary_meanings (",
        "  lemma_id, spanish, normalized_spanish, example, example_es,",
        "  level, source, legacy_vocabulario_id, is_active",
        ")",
        "select",
        "  v.lemma_id,",
        "  v.spanish,",
        "  lower(trim(v.spanish)),",
        "  v.example_sentence_de,",
        "  v.example_sentence_es,",
        "  v.level,",
        "  'manual_import',",
        "  v.id,",
        "  true",
        "from vocabulario v",
        "where v.lemma_id is not null",
        "  and v.primary_meaning_id is null",
        "  and not exists (",
        "    select 1 from vocabulary_meanings vm",
        "    where vm.legacy_vocabulario_id = v.id",
        "  );",
        "",
        "-- 4. Back-fill vocabulario.primary_meaning_id",
        "update vocabulario v",
        "set primary_meaning_id = vm.id",
        "from vocabulary_meanings vm",
        "where vm.legacy_vocabulario_id = v.id",
        "  and v.primary_meaning_id is null;",
        "",
        "-- 5. app_vocabulary_meanings",
        "insert into app_vocabulary_meanings (app_key, meaning_id, is_active)",
        "select 'imkontext', v.primary_meaning_id, true",
        "from vocabulario v",
        "where v.primary_meaning_id is not null",
        "  and not exists (",
        "    select 1 from app_vocabulary_meanings avm",
        "    where avm.meaning_id = v.primary_meaning_id",
        "      and avm.app_key = 'imkontext'",
        "  )",
        "on conflict do nothing;",
        "",
        "-- 6. text_version_meanings",
        "insert into text_version_meanings (text_version_id, meaning_id)",
        "select tvv.text_version_id, v.primary_meaning_id",
